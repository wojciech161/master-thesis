...
Background of these maps have visible moro effect and some distortions(dust). To eliminate this,
there was used a Gaussian Blur with a big radius - 15. Diameter of a bilateral filter was set to
20, its sigma space to 75 and sigma color to 150. To keep the shape of areas and remove all details
...
erosion and dilation application. Maximum numbers of colors was set to 30. It is enough for the
background. \\
...
Image contains small number of colors. Maximal number of colors was set to 12 and initial threshold
...
errors. Near roads and bigger text, areas were assigned to wrong groups. It is epecially visible on
the right center border of the map(it is marked in darker color). The same situation is visible
near text "Ślężański Park Krajobrazowy". It is caused by dilation and erosion algorithms
...
values some dots are still visible. To ensure quality and sharpness of details and text and also to
remove dots from moro effect, the threshold value was set to 100. Unsharp mask standard deviation is
...
Size of the areas was segmented properly. Almost all elements were segmented to proper groups. Text
and small details are visible, sharp and readable. Unfortunately moro effect in some places was too
difficult to segment out. It is especially visible near the lakes(shown in a Fig.\ref{pokojBadLake})
and in the town(visualized in a Fig.~\ref{pokojBadCity}).
...
Maps of Lubsza and Brzeg have some distortions. To eliminate them there were used filters with a
bigger parameter values. Gaussian blur was applied with radius set to 13. Bilateral filter was
called with diameter set to 10, sigma color and sigma space set to 150. Dilation and erosion was
applied five times with a size set to 1. Holes were removed using a window width set to 30. Maps
have maximum 12 colors. \\

Line swere deted with a threshold set to 100. To improve quality of details, there was applied an
unsharp mask with standard deviation set to 5 and denoising factor set to 35. Gaussian blur was
applied twice with radius set to 5. Bilateral filter was applied five times with diameter set to 30,
...
Major part of areas and details was segmented properly. Text and details are sharp and readable.
Many distortions were eliminated. Unfortunately area inside the city of Brzeg is segmented wrongly.
...
Last, the moro effect inside main road was not eliminated(shown in a Fig.~\ref{badBrzegRoad}).
...
Areas of this map were segmented very well. Also text and small details remained sharp and readable.
Unfortunately, the algorithm was not able to eliminate the distortion caused by the creased input.
...
In case of the map of Bydgoszcz, the result of the algorithm was worse than achieved in other
images. Shape of the background areas is correct in most of cases, but due to small resolution of
the map and a lot of distortions, many areas were assigned to wrong groups. It is shown in a
...
Quality of the output image was the worst in case of a map of Bydgoszcz, but it can be caused by the
...
lines - details are very small. In this algorithm there was used strong filtration. Radius of a
Gaussian filter was set to 13. In Bilateral filter diameter was set to 10, sigma color and sigma
space to 150. Dilation and erosion was applied 5 times with size set to 1. Window width for holes
removal was set to 20. Maximum number of background colors for these maps was set to 10. \\

Line detection algorithm is very sensitive. It has to find all thin lines and small details or
letters. Threshold was big - it was set to 150. To improve the image quality, standard deviation for
unsharp mask was set to 3 and denoising factor to 35. Gaussian blur was applied once with a radius
set to 3. Bilateral filter was applied twice with a diameter set to 10, sigma color to 70 and sigma
space to 10. Maximum number of colors was set to 11m initial threshold was 10 and minimal color area
was 500.

\subsubsection{Achieved results}

The map of Namysłów with an algorithm applied is shown in a Fig.~\ref{namyslowResult}.
...
Namysłów map was segmented very good. Areas have the same borders, almost all pixels were assigned
...
places color of the background was assigned wrongly. It can be seen in a Fig.~\ref{masywSlezyBadAreas}.
...
Thresholding for this map was set to 245. To improve image quality, there was used an unsharp mask
with standard deviation set to 1 and denosing algorithm with a factor value set to 20. Gaussian blur
was applied once with radius set to 1. Bilateral filter was applied twice with diameter set to 10,
sigma color to 70 and sigma space to 10. Maximum number of colors was set to 7, initial threshold
...
This map was segmented very well. The text and small elements are visible and readable. Almost all,
even very thin lines are visible and have proper color set. Only one visible error is in places,
where map was creased. The algorithm was not able to eliminate these distorions and recognized it
